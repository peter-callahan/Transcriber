...
for uuid, response_data in responses.items():
    ...
    if response_data.get('is_valid_json', False):
        ...
        folder_name = f"{entry_data['title'].replace(' ', '_')}_{entry_data['date'].replace(' ', '_')}"
        markdown_content = f"# {entry_data['title']}\n\n"
        markdown_content += f"**Tags:** {' '.join([f'#{tag}' for tag in entry_data['tags']])}\n\n"
        markdown_content += f"**Date:** {entry_data['date']}\n\n"

        # Add upload date if it exists
        if 'upload_date' in entry_data:
            markdown_content += f"**Upload Date:** {entry_data['upload_date']}\n\n"

        markdown_content += f"{entry_data['transcription']}\n"
    else:
    ...
    markdown_file_path = os.path.join(folder_path, "response.md")
    ...
    # Ensure image paths are prefixed with './' for relative paths
    image_paths = [f"./{image_path}" if not image_path.startswith(
        "./") else image_path for image_path in response_data.get("image_paths", [])]
